  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.body.hasOwnProperty('newQuantity')) {
      ...
      if (updatedQuantity !== currentItem.quantity) {
        ...
        changeLogs.push(`Quantity updated from ${currentItem.quantity} to ${updatedQuantity} on ${formatMYT(updatedAuditDate)}`);
      }
      ...
      if (parsedChange !== 0) {
        ...
        isQuantityUpdated = true;
        changeLogs.push(`Quantity changed from ${currentItem.quantity} to ${updatedQuantity} (Change: ${parsedChange}) on ${formatMYT(updatedAuditDate)}`);
      }
    }
...
app.put('/logs/:source/:id', authenticateJWT, asyncHandler(async (req, res) => {
  ...
  switch (source) {
    case 'transaction': {
      // If updated_by is missing, fallback to the authenticated user's username (if available)
      ...
      const userRes = await pool.query('SELECT user_id FROM users WHERE username = $1', [updated_by]);
      const user_id = userRes.rows[0]?.user_id;
      if (!user_id) {
        return res.status(400).json({ error: 'User not found for updated_by' });
      }
      query = `
  ...
  try {
    ...
    if (source === 'transaction') {
      ...
      // Ensure updated_by is preserved for subsequent edits
