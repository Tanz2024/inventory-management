...
const Reservation_status = ({ onClose, fetchItems }) => {
  ...
  const fetchItemsFromServer = async () => {
    try {
      const response = await fetch('https://ac2f-211-25-11-204.ngrok-free.app/admin-dashboard/items/reservations', {
        method: 'GET',
        credentials: 'include',
        headers: {
  ...
  const handleReservationUpdate = async (reservationId, action) => {
    ...
    const endpoint = `https://ac2f-211-25-11-204.ngrok-free.app/admin-dashboard/items/${reservationId}/${action}`;
    try {
