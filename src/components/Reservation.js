...
const Reservation = ({ onLogout,userId,username }) => {
  ...
  const handleConfirm = async (itemId) => {
    ...
    try {
      const response = await fetch(`https://e89d-211-25-11-204.ngrok-free.app/admin-dashboard/items/${itemId}/reserve`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': '1'
  ...
  const handleOcrSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://e89d-211-25-11-204.ngrok-free.app/upload', {
        method: 'POST',
        body: formData,
        credentials: 'include',
        headers: {
