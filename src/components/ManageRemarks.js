...
const ManageRemarks = ({ onClose, onUpdate }) => {
  ...
  const handleAddRemark = async () => {
    ...
    if (!trimmed) return;
    try {
      const resp = await fetch('https://ac2f-211-25-11-204.ngrok-free.app/dropdown-options/remarks', {
        method: 'POST',
        credentials: 'include',
        headers: commonHeaders,
  ...
  const handleDeleteRemark = async (remarkName) => {
    ...
    try {
      const resp = await fetch(
        `https://ac2f-211-25-11-204.ngrok-free.app/dropdown-options/remarks/${encodeURIComponent(remarkName)}`,
        {
          method: 'DELETE',
          credentials: 'include',
          headers: commonHeaders
  ...
  const handleConfirmRename = async (oldName) => {
    ...
    try {
      const resp = await fetch('https://ac2f-211-25-11-204.ngrok-free.app/dropdown-options/remarks/rename', {
        method: 'PATCH',
        credentials: 'include',
        headers: commonHeaders,
