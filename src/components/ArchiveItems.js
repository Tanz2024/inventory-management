...
const ArchivedItems = () => {
  ...
  const handleRestore = async (itemId) => {
    try {
      const response = await fetch('https://3ed2-211-25-11-204.ngrok-free.app/admin-dashboard/items/restore', {
        method: 'PATCH',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handlePermanentDelete = async (itemId) => {
    ...
    try {
      const response = await fetch('https://3ed2-211-25-11-204.ngrok-free.app/admin-dashboard/items/permanent', {
        method: 'DELETE',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
